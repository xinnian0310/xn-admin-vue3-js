...
/** 只认 attachments 列表 */
export function resolveAttachments(source) {
  if (!source) return []
...
export function toAttachmentItem(file) {
  return {
    name: file.name,
    path: file.path,
    size: file.size,
...
export function toAttachmentPayload(items) {
  const attachments = (items || [])
    .filter((item) => item?.name && item?.path)
    .map((item) => {
      const next = { name: item.name, path: item.path }
...
export function seedAttachmentOrders(items, pathOrder) {
  pathOrder.clear()
...
export function insertAttachmentByOrder(list, item, order, pathOrder) {
